...
with tab_desk:
    ...
    # ⚡ UNBREAKABLE NATIVE TELEMETRY DISPLAY MATRIX
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown(f"### 📊 Algorithmic Technical Parameter Matrix ({symbol_choice})")
    ...
    st.info(f"🧬 **Risk Sizing Recommendation Matrix:** Baseline lot size volume calculated at `{calculated_lots} Lots`")
...
with tab_journal:
    ...
    if saved_trades:
        ...
        if st.button("🗑️ WIPE PERSISTENT LEDGER RECORDS", type="secondary", use_container_width=True):
            clear_trade_database()
            st.rerun()
    else:
        st.info("No recorded trade footprints found inside system history cache files.")
...
with tab_rules:
    ...
    st.checkbox("Spread Protection Filter (Current Volatility Points Checked)", value=True, disabled=True)
    st.checkbox("Check Daily Absolute Target Threshold Drawdown Bounds (5.00% Limit Protect)", value=True, disabled=True)
    st.checkbox("RSI Overbought Exclusion Filter Window Threshold Trigger (< 60.00 Limit)", value=True, disabled=True)
    st.checkbox("RSI Oversold Demand Accumulation Filter Window Trigger (> 40.00 Floor)", value=True, disabled=True)
